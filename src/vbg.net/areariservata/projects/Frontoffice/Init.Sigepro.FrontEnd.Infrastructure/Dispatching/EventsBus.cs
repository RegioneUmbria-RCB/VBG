	...
	public class EventsBus : ICommandSender, IEventPublisher
	{
		...
		public void Publish<T>(T @event) where T : Event
		{
			if(_log.IsDebugEnabled)
				_log.Debug($"Inizio pubblicazione dell'evento {typeof(T)}, dati dell'evento: {JsonConvert.SerializeObject(@event)}");
		...
		{
			if (_log.IsDebugEnabled)
                _log.Debug($"Inizio pubblicazione del comando {typeof(T)}, dati del comando: {JsonConvert.SerializeObject(command)}");

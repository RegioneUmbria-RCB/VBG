...
using it.regulus.osa.secretcode;
using log4net;
using System;
    ...
    internal class PayServerClientWrapper : IPayServerClient
    {
        ...
        public string GeneraUrlRedirect(PaymentRequest request)
        {
            try
            {
                ...
                var result = client.PS2S_PC_Request2RID(xmlRequest, this._settings.ComponentName, new java.util.Date());

                if (result != 0)
                {
                    var errMsg = client.PS2S_ErrorDesc();
        ...
        public string EstraiBuffer(string buffer)
        {
            ...
            var r = client.PS2S_PC_PID2Data(buffer ?? String.Empty, this._settings.ComponentName, new java.util.Date(), this._settings.WindowMinutes);
        ...
        public string GetStatoPagamento(MIPPaymentStatusRequest request)
        {
            try
            {
                ...
                if (!uw.doGet())
                {
                    var errMsg = String.Format("Impossibile chiamare l'indirizzo {0} per ottenere informazioni sul pagamento", urlChiamata);
                    throw new PaymentServiceException(errMsg);
                }

                var buffer = EstraiBuffer(uw.getBufferOut());
        ...
        private URLWorker CreaUrlWorker(string url)
        {
            ...
            if (!String.IsNullOrEmpty(this._settings.ProxyAddress))
            {
                ...
                urlWorker.setProxyServer(parts[0]);
                urlWorker.setProxyPort(parts[1]);
            }

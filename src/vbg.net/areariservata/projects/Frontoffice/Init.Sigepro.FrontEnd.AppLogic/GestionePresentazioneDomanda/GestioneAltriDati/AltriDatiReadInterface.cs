...
namespace Init.Sigepro.FrontEnd.AppLogic.GestionePresentazioneDomanda.GestioneAltriDati
{
	public class AltriDatiReadInterface : IAltriDatiReadInterface
	{
		...
		public AltriDatiReadInterface(PresentazioneIstanzaDataKey dataKeyDomanda, PresentazioneIstanzaDbV2 database)
		{
            ...
            PreparaIdDomandaCollegata();

		}

        private void PreparaIdDomandaCollegata()
        {
            if (this._database.ISTANZE.Count == 0 || this._database.ISTANZE[0].IsIdDomandaCollegataNull())
                return;

            this._idDomandaCollegata = this._database.ISTANZE[0].IdDomandaCollegata;
        }
		...
		private void PreparaEventi()
		{
			...
			var testoEvento1  = String.Format("{0} in data {1} alle ore {2} ha trasferito l'istanza all'ente",
												GetNominativoUtenteCheEffettuaInvio(),
												DateTime.Now.ToString("dd/MM/yyyy"),
												DateTime.Now.ToString("HH:mm"));

			var codiceEvento2 = Constants.IdEventoRecapitata;
			var testoEvento2  = String.Format("In data {0} alle ore {1} l'istanza e' stata trasferita all'ente",
												DateTime.Now.ToString("dd/MM/yyyy"),
												DateTime.Now.ToString("HH:mm"));
		...
		public DatiIntervento Intervento => this._intervento;
		public string DescrizioneLavori => this._descrizioneLavori; 
		public string DenominazioneAttivita => this._denominazioneAttivita; 
		public string Note => this._note; 
		public IEnumerable<Evento> Eventi => _eventi;
		public string DomicilioElettronico => this._domicilioElettronico;
		public string IdentificativoDomanda => this._identificativoDomanda;
		public string CodiceComune => this._codiceComune;
		public string AliasComune =>  this._aliasComune;
        public string Software => this._software; 
		public int IdPresentazione => this._idPresentazione;
		public bool FlagPrivacy => this._flagPrivacy;
		public int? AttivitaAtecoPrimaria  => this._attivitaAtecoPrimaria;
        public int? IdDomandaCollegata => this._idDomandaCollegata;

        public string NaturaBase
        {
            get
            {
                if (this._database.ISTANZE.Count == 0 || this._database.ISTANZE[0].IsNaturaBaseNull())
                {
                    return String.Empty;
                }

                return this._database.ISTANZE[0].NaturaBase;
            }
        }

...
namespace Init.Sigepro.FrontEnd.AppLogic.GestionePresentazioneDomanda.GestioneAllegati
{
	...
	{
		void Elimina(int id);
        void EliminaByCodiceOggetto(int codiceOggetto);
	...
	public class AllegatiWriteInterface : IAllegatiWriteInterface
	{
        ...
        public void EliminaByCodiceOggetto(int codiceOggetto)
        {
            var rows = this._database.Allegati.Where( x => x.CodiceOggetto == codiceOggetto).ToList();

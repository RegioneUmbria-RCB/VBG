...
namespace Init.Sigepro.FrontEnd.AppLogic.GestionePresentazioneDomanda.GestioneDocumenti
{
	public interface IDocumentiWriteInterface
	{
        ...
        void ForzaEliminazioneDocumentoDaCodiceOggetto(int codiceOggetto);

        // Allegati dell'intervento
        void EliminaDocumentiInterventoSenzaAllegati();
		void AggiungiOAggiornaDocumentoIntervento(int codiceDocumento, string descrizione, string linkInformazioni, int? codiceOggetto, bool richiesto, bool richiedeFirma, string tipoDownload, int ordine, string nomeFileModello, bool riepilogoDomanda, int codiceCategoria, string descrizioneCategoria, string note, int dimensioneMassima, string estensioniAmmesse);
        void CopiaDocumentiDaDomanda(DomandaOnline domandaOrigine, IEnumerable<int> idAllegatiDiOrigine);
        void AggiungiDocumentoInterventoLibero(string descrizione, int codiceOggetto, string nomeFile, int codiceCategoria = -1, string descrizioneCategoria = "Altri allegati", bool isFirmatoDigitalmente = false);
		void AggiungiDocumentoSchedaDinamica(string descrizione, int codiceOggetto, string nomeFile);

		// Allegati endo
		void AggiungiOAggiornaDocumentoEndo(int codiceDocumento, string descrizione, string linkInformazioni, int? idModello, bool richiesto, bool richiedeFirma, string tipoDownload, int codiceEndo, int ordine, string nomeFileModello, string note, int dimensioneMassima, string estensioniAmmesse);

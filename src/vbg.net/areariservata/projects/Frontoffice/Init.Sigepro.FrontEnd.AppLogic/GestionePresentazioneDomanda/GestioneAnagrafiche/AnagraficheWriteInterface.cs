...
using System.Data;
using static Init.Sigepro.FrontEnd.AppLogic.ObjectSpace.PresentazioneIstanza.PresentazioneIstanzaDataSet;
...
//			public const string IdColonnaChiavePrimaria = "ANAGRAFE_PK";
        ...
        public void CopiaAnagraficheDaDomanda(DomandaOnline domandaOrigine, IEnumerable<IAnagraficaDaCopiare> anagrafiche)
        {
            var databaseOrigine = (domandaOrigine.WriteInterface.Anagrafiche as AnagraficheWriteInterface)._database;

            this._database.Procure.Clear();
            this._database.ANAGRAFE.Clear();
            


            foreach (var a in anagrafiche)
            {
                var anagraficaOrigine = databaseOrigine.ANAGRAFE.FindByANAGRAFE_PK(a.Id);

                var newRow = this._database.ANAGRAFE.NewRow();

                foreach(var column in anagraficaOrigine.Table.Columns.Cast<DataColumn>())
                {
                    newRow[column.ColumnName] = anagraficaOrigine[column.ColumnName];
                }

                this._database.ANAGRAFE.Rows.Add(newRow);

                var riga = this._database.ANAGRAFE.FindByANAGRAFE_PK(a.Id);

                riga.TIPOSOGGETTO = a.TipoSoggetto;
                riga.DescrSoggetto = a.DescrizioneTipoSoggetto;

                riga.FlagRichiedeAnagraficaCollegata = a.RichiedeAnagraficaCollegata;

                if (!a.RichiedeAnagraficaCollegata)
                {
                    riga.SetIdAnagraficaCollegataNull();
                }
            }

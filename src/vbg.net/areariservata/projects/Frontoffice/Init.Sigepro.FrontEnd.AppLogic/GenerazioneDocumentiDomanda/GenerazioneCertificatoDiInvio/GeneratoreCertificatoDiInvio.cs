...
namespace Init.Sigepro.FrontEnd.AppLogic.GenerazioneDocumentiDomanda.GenerazioneCertificatoDiInvio
{
	public class GeneratoreCertificatoDiInvio
	{
		...
		public BinaryFile GeneraCertificatoDiInvio(DomandaOnline domanda, string idDomandaBackoffice, IStrategiaIndividuazioneCertificatoInvio strategiaIndividuazioneRiepilogo)
		{
			var idDomandaFrontoffice = domanda.DataKey.ToSerializationCode();

			if (!strategiaIndividuazioneRiepilogo.IsCertificatoDefinito)
			{
                _log.ErrorFormat("non è stato possibile generare un certificato di invio per l'istanza con id domanda {0} e codice istanza {1}", idDomandaFrontoffice, idDomandaBackoffice);
				return null;
			}

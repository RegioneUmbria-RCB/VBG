// -----------------------------------------------------------------------
// <copyright file="MappaturaCampiSchedeImu.cs" company="">
...
namespace Init.Sigepro.FrontEnd.AppLogic.GestioneBari.Imu
{
	...
	/// TODO: Update summary.
	/// </summary>
	public class MappaturaCampiSchedeImu
	{
		...
		internal IEnumerable<CampoDinamicoMappato> Mappa(DatiContribuenteImuDto datiContribuenteDto)
		{
			var abitazione = datiContribuenteDto.ListaImmobili.Where(x => x.TipoImmobile == TipoImmobileEnum.Abitazione).First();
            var pertinenze = datiContribuenteDto.ListaImmobili.Where(x => x.TipoImmobile == TipoImmobileEnum.Pertinenza);

			var immobili = new List<ImmobileImuDto>();
			...
			var utenzaVuota = ImmobileImuDto.Vuoto();
		...
		private IEnumerable<CampoDinamicoMappato> MappaInternal(int idx, ImmobileImuDto datiUtenza)
		{
			...
			yield return new CampoDinamicoMappato(_categoria[idx], datiUtenza.CategoriaCatastale);

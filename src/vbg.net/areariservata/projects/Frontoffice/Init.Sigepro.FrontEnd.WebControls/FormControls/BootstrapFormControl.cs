...
namespace Init.Sigepro.FrontEnd.WebControls.FormControls
{
    ...
    public abstract class BootstrapFormControl<T> : WebControl, IControlWithLabel, IBootstrapFormControl where T : WebControl
    {
        ...
        protected string GroupCssClass
        {
            get { object o = this.ViewState["GroupCssClass"]; return o == null ? String.Empty : (string)o; }
            set { this.ViewState["GroupCssClass"] = value; }
        }


        protected virtual IEnumerable<KeyValuePair<string, string>> GetGroupAttributes()
        {
        ...
        protected override void Render(System.Web.UI.HtmlTextWriter writer)
        {
            ...
            if (!String.IsNullOrEmpty(this.GroupCssClass))
            {
                cssClass += $" {this.GroupCssClass} ";
            }

            writer.WriteBeginTag("div");
            writer.WriteAttribute("class", cssClass);

            this.GetGroupAttributes().ToList().ForEach(x => writer.WriteAttribute(x.Key, x.Value));

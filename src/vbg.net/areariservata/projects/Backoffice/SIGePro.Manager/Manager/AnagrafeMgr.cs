	...
	public class AnagrafeMgr : BaseManager, IAnagrafeManager
	{
		...
		private void ChildInsert(Anagrafe p_class)
		{
			...
			foreach (AnagrafeDyn2ModelliT id2mt in p_class.AnagrafeDyn2ModelliT)
			{
				...
				var loader = new ModelloDinamicoLoader(dap, p_class.IDCOMUNE, ModelloDinamicoLoader.TipoModelloDinamicoEnum.Backoffice);
        ...
        public Anagrafe GetByUserId(string idComune, string userId, TipoPersona tipoPersona)
		{
			...
			try
			{
                ...
                var sql1 = @"select 
							...
							where 
								...
								db.Specifics.UCaseFunction( "STRONG_AUTH_ID" ) + " = {1} and tipoanagrafe={2}";
                ...
                var sql2 = @"select 
							...
							where 
								idcomune={0} and " +
                                db.Specifics.NvlFunction("FLAG_DISABILITATO", "0") + " = 0 and " +
                                    db.Specifics.UCaseFunction("CODICEFISCALE") + " = {1} and tipoanagrafe={2}";

                sql1 = PreparaQueryParametrica(sql1, "idComune", "codicefiscale", "tipoanagrafe");
                sql2 = PreparaQueryParametrica(sql2, "idComune", "codicefiscale", "tipoanagrafe");
                ...
                foreach (var sql in new[] { sql1, sql2 })
                {
                    using (IDbCommand cmd = db.CreateCommand(sql))
                    {
                        ...
                        cmd.Parameters.Add(db.CreateParameter("tipoanagrafe", tipoPersona == TipoPersona.Fisica ? "F" : "G"));

                        var anagrafe = db.GetClass<Anagrafe>(cmd);

    ...
    public partial class AlberoProcMgr
    {
        ...
        public List<SchedaDinamicaInterventoDto> GetSchedeDinamicheFoDaIdIntervento(string idComune, int idIntervento)
        {
            ...
            try
            {
                ...
                {
                    if (schedeIntervento.FirstOrDefault(x => x.Id == schedaProcedura.Id) == null)
                    {
                ...
                using (IDbCommand cmd = db.CreateCommand(sqlSelezioneEndo))
                {
                    ...
                    using (var dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            ...
                            var ordineFamiglia = dr.GetInt("famigliaOrdine").GetValueOrDefault(999);
                            var ordineTipo = dr.GetInt("tipoEndoOrdine").GetValueOrDefault(999);
        ...
        {
            var sql = $"select FLAG_UNICA_DOMANDA from alberoproc where idcomune={this.db.Specifics.QueryParameterName("idComune")} and sc_id={this.db.Specifics.QueryParameterName("idIntervento")}";

            var flag = this.db.ExecuteScalar(sql, 0, mp =>
            {
...
where 
    istanze.idcomune={this.db.Specifics.QueryParameterName("idComune")} and 
    istanze.codiceinterventoproc={this.db.Specifics.QueryParameterName("idIntervento")} and
    {db.Specifics.UCaseFunction("anagrafe.codicefiscale")} = {this.db.Specifics.QueryParameterName("codiceFiscaleRichiedente")}";

...
namespace Init.SIGePro.Manager.Logic.Visura
{
	...
	public class VisuraPraticheManager
	{
		...
		public ListaPratiche GetListaPratiche(RichiestaListaPratiche richiesta)
		{
			...
			var filtriListaPratiche = new List<string>(GetFiltriRicercaSoggettiInListaPratiche(richiesta));

            var filtriSoggettiCollegati = new List<string>(GetFiltriRicercaSoggettiInSoggettiCollegati(richiesta));
			...
			if (!StringChecker.IsStringEmpty(richiesta.Oggetto))
				filtriListaPratiche.Add("VW_LISTAPRATICHE.OGGETTO like '" + SafeString(richiesta.Oggetto).ToUpper() + "%'");
            ...
            sbSql.Append(" order by istanze.data desc, istanze.dataprotocollo desc, IDPRATICA DESC");
			...
			try
			{
				if (limiteRecords > 0)
				{
                    ...
                    m_logger.DebugFormat("SQL conteggio lista: {0}", sqlCount);
				...
				sqlTmp = "select distinct vw_listapratiche.*, istanze.* " + sqlTmp;

                m_logger.DebugFormat("SQL selezione lista: {0}", sqlTmp);

				using (IDbCommand cmd = m_database.CreateCommand(sqlTmp))
				{
                    var results = new List<IdentificativoPratica>();

					using (IDataReader reader = cmd.ExecuteReader())
					{
						while (reader.Read())
						{
                            var datiPratica = new IdentificativoPratica
                            {
                                DatiPratica = new DatiPratica
                                {
                                    CodSportelloBack = reader["Software"].ToString(),
                                    DesSportelloBack = reader["DescSoftware"].ToString(),
                                    CodStatoPratica = reader["CodStatoPratica"].ToString(),
                                    DataPresentazione = String.IsNullOrEmpty(reader["DataPresentazione"].ToString()) ? String.Empty : Convert.ToDateTime(reader["DataPresentazione"]).ToString("dd/MM/yyyy"),
                                    DataProtocollo = String.IsNullOrEmpty(reader["DataProtocollo"].ToString()) ? String.Empty : Convert.ToDateTime(reader["DataProtocollo"]).ToString("dd/MM/yyyy"),
                                    DescrizioneIntervento = reader["descrizioneintervento"].ToString(),
                                    IdPratica = reader["IdPratica"].ToString(),
                                    NumeroPratica = reader["NumeroPratica"].ToString(),
                                    Zonizzazione = reader["ZONIZZAZIONE"].ToString(),
                                    DescrizioneProcedura = reader["PROCEDURA"].ToString(),
                                    NumeroProtocollo = reader.ReadString("NumeroProtocollo"),
                                    Oggetto = reader.ReadString("Oggetto"),
                                    ResponsabileProcedimento = reader["responsabile"].ToString(),
                                    StatoPratica = reader["StatoPratica"].ToString(),
                                },

                                Localizzazione = new Localizzazione
                                {
                                    Civico = reader.ReadString("pr_civico"),
                                    CodCivico = reader.ReadString("pr_codcivico"),
                                    CodViario = reader.ReadString("pr_codviario"),
                                    Indirizzo = reader.ReadString("pr_Indirizzo")
                                },

                                RifCatastale = new RifCatastale
                                {
                                    Foglio = reader.ReadString("Foglio"),
                                    Particella = reader.ReadString("Particella"),
                                    Subalterno = reader.ReadString("Sub"),
                                    TipoCatasto = reader.ReadString("TipoCatasto")
                                },

                                Soggetto = new Soggetto
                                {
                                    Cap = reader.ReadString("Cap"),
                                    Citta = reader.ReadString("Citta"),
                                    CodFiscale = reader.ReadString("codicefiscale"),
                                    Indirizzo = reader.ReadString("Indirizzo"),
                                    Localita = reader.ReadString("Localita"),
                                    Nominativo = reader.ReadString("Nominativo"),
                                    PartitaIva = reader.ReadString("PartitaIva"),
                                    Provincia = reader.ReadString("Provincia"),
                                    TipoRapporto = reader.ReadString("TipoRapporto")
                                },

                                Azienda = new Soggetto
                                {
                                    Nominativo = reader.ReadString("AZ_NOMINATIVO"),
                                    CodFiscale = reader.ReadString("AZ_CODICEFISCALE"),
                                    PartitaIva = reader.ReadString("AZ_PARTITAIVA")
                                }
                            };
                    ...
                    var retVal = new ListaPratiche
                    {
                        EnteTitolare = new EnteTitolare
                        {
                            Sportello = new Sportello(),
                            Ente = new Ente
                            {
                                CodEnte = richiesta.CodEnte,
                                DesEnte = ""
                            }
                        }
                    };
					...
					if (results.Count > 0)
					{
                        retVal.Pratiche = results.ToArray();
        ...
        public RisultatoVisuraListaDto GetListaPraticheV2(RichiestaListaPratiche richiesta)
        {
            // IdComune e software devono essere sempre specificati
            if (String.IsNullOrEmpty(richiesta.CodEnte))
                throw new Exception("Codice ente non specificato");
            ...
            if (!StringChecker.IsStringEmpty(richiesta.NumeroPratica))
                filtriListaPratiche.Add($"(VW_LISTAPRATICHE.NUMEROPRATICA ='{SafeString(richiesta.NumeroPratica)}' or VW_LISTAPRATICHE.NUMEROPRATICA like '{SafeString(richiesta.NumeroPratica)}/%')");
            ...
            try
            {
                if (limiteRecords > 0)
                {
                    ...
                    if (intCount > limiteRecords)
                    {
                        ...
                        return new RisultatoVisuraListaDto
                        {
                            LimiteRecordsSuperato = true
                        };
                ...
                using (IDbCommand cmd = m_database.CreateCommand(sqlTmp))
                {
                    var istanze = m_database.GetClassList<VisuraListItemDto>(cmd, new GetClassListFlags(useForeignEnum.Recoursive));

                    return new RisultatoVisuraListaDto
                    {
                        LimiteRecordsSuperato = false,
                        ListaPratiche = istanze
                    };
		...
		protected IEnumerable<string> GetFiltriRicercaSoggettiInSoggettiCollegati(RichiestaListaPratiche richiesta)
		{
			if (richiesta.CodFiscale != null)
			{
				foreach (RichiestaListaPraticheCodFiscale cf in richiesta.CodFiscale)
				{
					...
					sb.Append($" ( ANAGRAFE.CODICEFISCALE='{SafeString(cf.Value)}' " );

					if (cf.cercaAncheComePartitaIva)
						sb.AppendFormat($" OR ANAGRAFE.PARTITAIVA='{SafeString(cf.Value)}' ");
			...
			if (richiesta.PartitaIva != null)
			{
				foreach (RichiestaListaPratichePartitaIva pi in richiesta.PartitaIva)
				{
					...
					StringBuilder sb = new StringBuilder();
					sb.Append($" ( ANAGRAFE.PARTITAIVA='{SafeString(pi.Value)}' ");

					if (pi.cercaAncheComeCodiceFiscale)
						sb.Append($" OR ANAGRAFE.CODICEFISCALE='{SafeString(pi.Value)}' ");
		...
		/// <param name="richiesta">Parametri di ricerca dell'istanza</param>
		/// <returns>Lista di filtri da applicare alla tabella Soggetticollegati</returns>
		protected IEnumerable<string> GetFiltriRicercaSoggettiInListaPratiche(RichiestaListaPratiche richiesta)
		{
			if (richiesta.CodFiscale != null)
			{
				foreach (RichiestaListaPraticheCodFiscale cf in richiesta.CodFiscale)
				{
					...
					if (cf.cercaComeRichiedente)
                    {
                        ...
                        subFilters.Add($"{m_database.Specifics.UCaseFunction("VW_LISTAPRATICHE.NOMINATIVO")} like '%{SafeString(cf.Value.ToUpper())}%'");
                    }
					...
					if (cf.cercaComeTecnico)
                    {
                        ...
                        subFilters.Add($"{m_database.Specifics.UCaseFunction("VW_LISTAPRATICHE.TEC_NOMINATIVO")} like '%{SafeString(cf.Value.ToUpper())}%'");
                    }
					...
					if (cf.cercaComeAzienda)
                    {
                        ...
                        subFilters.Add($"{m_database.Specifics.UCaseFunction("VW_LISTAPRATICHE.AZ_NOMINATIVO")} like '%{SafeString(cf.Value.ToUpper())}%'");
                    }
					...
					if (subFilters.Count > 0)
						yield return JoinFilters(subFilters, "OR");
				}
			}
		...
		protected string JoinFilters(List<string> subFilters, string operation)
		{
			...
			if (subFilters.Count > 0)
			{
                ...
                // }
                cfSb.Append(String.Join(strOperation, subFilters.ToArray()));
		...
		protected string CreaFiltroPerCodiceFiscalePartitaIva(string valore, bool cercaInEntrambi, string colonna1, string colonna2)
		{
			...
			sb.Append($" ( VW_LISTAPRATICHE.{colonna1} = '{SafeString(valore)}'");

			if (cercaInEntrambi)
				sb.Append($" OR VW_LISTAPRATICHE.{colonna2} = '{SafeString(valore)}'");

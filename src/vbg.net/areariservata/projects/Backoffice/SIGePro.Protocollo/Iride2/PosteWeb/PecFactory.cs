...
namespace Init.SIGePro.Protocollo.Iride2.PosteWeb
{
    ...
    {
        public static IPec Create(ProtocolloIrideEnumerators.VersioneEnum tipo, IEnumerable<string> seriali, string[] destinatari, string codiceAoo, bool invioInterop, ProtocolloLogs logs, ProtocolloSerializer serializer)
        {
            logs.InfoFormat("VERSIONE: {0}", tipo.ToString());
            if (tipo == ProtocolloIrideEnumerators.VersioneEnum.J_IRIDE)
                return new JIridePec(destinatari, seriali, codiceAoo, logs, serializer, invioInterop);

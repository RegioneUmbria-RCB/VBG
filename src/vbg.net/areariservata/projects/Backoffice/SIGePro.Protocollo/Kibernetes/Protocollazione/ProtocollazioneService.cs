    ...
    public class ProtocollazioneService
    {
        ...
        public void AggiungiAllegato(ProtocolloAllegati allegato, long numeroProtocollo, short annoProtocollo, VerticalizzazioniConfiguration vert, bool isPrincipale)
        {
            try
            {
                using (var ws = CreaWebService())
                {
                    using (OperationContextScope scope = new OperationContextScope(ws.InnerChannel))
                    {
                        ...
                        _logs.InfoFormat("INSERIMENTO DELL'ALLEGATO CODICE {0}, NOME {1}, AL PROTOCOLLO NUMERO {2}, ANNO {3}, PRINCIPALE {4} AVVENUTO CON SUCCESSO", allegato.CODICEOGGETTO, allegato.NOMEFILE, numeroProtocollo, annoProtocollo, isPrincipale);
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                _logs.WarnFormat("ERRORE GENERATO DURANTE L'INSERIMENTO DELL'ALLEGATO FILE: {0}, DETTAGLIO ERRORE: {1}", allegato.NOMEFILE, ex.Message);
            }
        }
    }
}

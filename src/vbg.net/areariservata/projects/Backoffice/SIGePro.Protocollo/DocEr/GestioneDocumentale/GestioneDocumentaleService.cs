            ...
            try
            {
                ...
                using (var ws = CreaWebService())
                {
                    ...
                    foreach (var doc in docsAllegati)
                    {
                        ...
                        _logs.Info($"PARSING DEL DOCUMENTO CON ID {responseAllegato}");
                        doc.ID = ParseIdDocumento(responseAllegato);
                        _logs.Info($"PARSING DEL DOCUMENTO CON ID {responseAllegato} AVVENUTO CORRETTAMENTE");
                    }

                    _logs.Info($"AGGIUNTA DEGLI ALLEGATI [{String.Join(",", rel)}] ALL'UNITA' DOCUMENTARIA {docPrimario}");
                    ws.addRelated(_token, docPrimario, rel.ToArray());
                    _logs.Info($"AGGIUNTA DEGLI ALLEGATI ALL'UNITA' DOCUMENTARIA {docPrimario} AVVENUTA CON SUCCESSO");
                }
            }
            catch (System.Exception ex)
            {

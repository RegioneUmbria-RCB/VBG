...
namespace Init.SIGePro.Protocollo.SiprWebTest.Protocollazione.MittentiDestinatari
{
    public class MittentiDestinatariPartenza : BaseMittentiDestinatari, IMittentiDestinatari
    {
        ...
        private string[] GetDestinatari()
        {
            ...
            DatiProto.AnagraficheProtocollo.Where(x => x.ModalitaTrasmissione != _codiceCC || String.IsNullOrEmpty(_codiceCC)).ToList()
                                           .ForEach((x => rVal.Add(String.Format("{0} {1} CF:{2} PIVA:{3}", x.NOME, x.NOMINATIVO, x.CODICEFISCALE, x.PARTITAIVA).TrimStart())));

            DatiProto.AmministrazioniProtocollo.Where(x => x.ModalitaTrasmissione != _codiceCC || String.IsNullOrEmpty(_codiceCC)).ToList()

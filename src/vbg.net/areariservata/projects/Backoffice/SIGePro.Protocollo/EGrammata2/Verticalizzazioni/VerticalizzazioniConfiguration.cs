...
namespace Init.SIGePro.Protocollo.EGrammata2.Verticalizzazioni
{
    public class VerticalizzazioniConfiguration
    {
        ...
        public string[] Fascicolo { get; private set; }
        ...
        private void VerificaIntegritaParametri(VerticalizzazioneProtocolloEgrammata2 paramVert)
        {
            try
            {
                ...
                if (String.IsNullOrEmpty(paramVert.Fascicolo))
                    throw new Exception("PARAMETRO FASCICOLO NON VALORIZZATO");
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        private void EstraiParametri(VerticalizzazioneProtocolloEgrammata2 vert)
        {
            try
            {
                ...
                string fascicolo = vert.Fascicolo.Replace("#ANNO#", DateTime.Now.Year.ToString());
                this.Fascicolo = fascicolo.Split('|');

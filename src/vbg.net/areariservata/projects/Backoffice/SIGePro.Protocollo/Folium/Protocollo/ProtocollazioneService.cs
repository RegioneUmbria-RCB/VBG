...
namespace Init.SIGePro.Protocollo.Folium.Protocollo
{
    public class ProtocollazioneService : IProtocollazione
    {
                ...
                if (_info.Regole.IsContenuto)
                {
                    ...
                    if (allegati.Count() > 0)
                    {
                        ...
                        if (_info.Regole.EstensioniAccettateFileContenuto != null)
                        {
                            ...
                            var extPrinc = Path.GetExtension(allegatoPrincipale.NOMEFILE);
        ...
        public void Assegna(long idProtocollo)
        {
            ...
            if (!String.IsNullOrEmpty(_info.Smistamento) && _info.Flusso != FlussoAdapter.Constants.COD_PARTENZA_FOLIUM)
            {
                var request = new Assegnazione
                {
                    codiceAssegnazione = _info.Smistamento,
                    idProtocollo = idProtocollo,
                    ufficioAssegnatario = (_info.Flusso == FlussoAdapter.Constants.COD_INTERNO_FOLIUM) ? _info.RuoloDestinatarioInterno : _info.Ruolo,

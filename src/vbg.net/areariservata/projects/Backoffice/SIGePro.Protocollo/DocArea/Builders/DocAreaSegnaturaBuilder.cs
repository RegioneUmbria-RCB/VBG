...
using Init.SIGePro.Protocollo.DocArea.Builders.ClassificazioneFascicolazione;
...
namespace Init.SIGePro.Protocollo.DocArea.Builders
{
    public class DocAreaSegnaturaBuilder
    {
            ...
            ProtocolloSerializer serializer, List<ProtocolloAllegati> listAllegati)
        {
            ...
            this._listAllegati = listAllegati;
            this._tipoFornitore = TipoFornitoreProtocolloFactory.Create(_configuration);
            ...
            if (configuration.VertParams.MultiMittenteDestinatario)
                this._personeBuilder = new DocAreaSegnaturaMultiPersonaBuilder(logs);
            else
                this._personeBuilder = new DocAreaSegnaturaPersonaBuilder(logs);
        }
        ...
        private Intestazione CreaIntestazione()
        {
            ...
            var factoryClassFasc = FascicolazioneFactory.Create(_configuration.Classifica, _configuration.Flusso, _configuration.VertParams, _configuration.Provenienza, _configuration.TipoInserimento);
            var classifica = factoryClassFasc.GetClassificazione();
            var fascicolo = factoryClassFasc.GetFascicolo();

            if (classifica != null)
            {
                intestazione.Classifica = classifica;
            }

            if (fascicolo != null)
            {
                intestazione.Fascicolo = fascicolo;
            }

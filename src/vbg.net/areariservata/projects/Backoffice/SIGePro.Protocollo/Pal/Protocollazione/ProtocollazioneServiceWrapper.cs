...
namespace Init.SIGePro.Protocollo.Pal.Protocollazione
{
    public class ProtocollazioneServiceWrapper : BaseProtocolloServiceWrapper
    {
        ...
        public void InviaPec(string idProtocollo)
        {
            ...
            {
                _logs.WarnFormat("ERRORE AVVENUTO DURANTE L'INVIO PEC, ERRORE: {0}", ex.Message);
            }
        }

        public RootObject Protocolla(ProtocollazioneType request)
        {
            var uri = new Uri(new Uri(_baseUrlWs), _metodoCreaProtocollo);

            HttpWebRequest requestToServerEndpoint = (HttpWebRequest)WebRequest.Create(uri.ToString());
            ...
            catch (WebException e)
            {
                using (WebResponse response = e.Response)
                {
                    ...
                    using (var reader = new StreamReader(data))
                    {
                        ...
                        throw new Exception($"ERRORE GENERATO DURANTE LA PROTOCOLLAZIONE, {text}", e);
                    }
                }
            }
        }

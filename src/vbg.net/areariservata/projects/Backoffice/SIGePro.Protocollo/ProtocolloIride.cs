...
using Init.SIGePro.Protocollo.Iride.Fascicolazione;
    ...
    public class PROTOCOLLO_IRIDE : ProtocolloBase
    {
        ...
        public override DatiProtocolloFascicolato IsFascicolato(string idProtocollo, string annoProtocollo, string numeroProtocollo)
        {
            ...
            try
            {
                ...
                {
                    _protocolloIrideService = ProtocolloIrideFactory.Create(_vert.CodiceAmministrazione, _proxyProtIride);
        ...
        private DatiFascicolo FascicolaProtocollo(Fascicolo fascicolo)
        {
            ...
            if (this.DatiProtocollo.TipoAmbito == ProtocolloEnum.AmbitoProtocollazioneEnum.NESSUNO)
            {
                ...
                EsitoOperazione esito = _proxyFascIride.FascicolaDocumento(iFascicoloId, iDocumentoId, _vert.AggiornaClassifica, Operatore.ToUpper(), Ruolo, _vert.CodiceAmministrazione, string.Empty);
        ...
        */
        ...
        public override DatiProtocolloRes CreaCopie()
        {
            ...
            try
            {
                ...
                using (_proxyProtIride = new ProxyProtIride(_vert.Url, ProxyAddress))
                {
                    ...
                    _protocolloLogs.DebugFormat("Crea copie - Recupero dei dati dell'istanza, codice istanza: {0}, idcomune: {1}", DatiProtocollo.CodiceIstanza, DatiProtocollo.IdComune);
                    var ist = new IstanzeMgr(this.DatiProtocollo.Db).GetById(DatiProtocollo.IdComune, Convert.ToInt32(DatiProtocollo.CodiceIstanza));
                    _protocolloLogs.DebugFormat("Crea copie - dati dell'istanza recuperati, codice istanza: {0}, idcomune: {1}", DatiProtocollo.CodiceIstanza, DatiProtocollo.IdComune);

                    if (ist == null)
                        throw new Exception(String.Format("L'ISTANZA {0} CON IDCOMUNE {1} NON ESISTE", DatiProtocollo.CodiceIstanza, DatiProtocollo.IdComune));
                    ...
                    DocumentoOut docOut = LeggiProtocolloDocumento(ist.FKIDPROTOCOLLO, ist.DATAPROTOCOLLO.Value.Year.ToString(), ist.NUMEROPROTOCOLLO);
                    ...
                    var creaCopie = new CreaCopieService(_vert.Url, ProxyAddress, _protocolloLogs, _protocolloSerializer);
                    var creaCopieOut = creaCopie.CreaCopie(uo: amm.PROT_UO,
                                                            ...
                                                            codiceEnte: _vert.CodiceAmministrazione);
        ...
        private CreaCopieIn CreaCopieIn()
        {
            ...
            var docOut = LeggiProtocolloDocumento(this.DatiProtocollo.Istanza.FKIDPROTOCOLLO, this.DatiProtocollo.Istanza.DATAPROTOCOLLO.Value.Year.ToString(), this.DatiProtocollo.Istanza.NUMEROPROTOCOLLO);
        ...
        private void InviaPec(DatiProtocolloIn protoIn, ProtocolloOut protoOut, string ruolo)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        var pec = PecFactory.Create(_vert.Versione, seriali, listaPec, _vert.Aoo, _protocolloLogs, _protocolloSerializer);
                        pec.Invia(_vert.UrlPec, ProxyAddress, protoOut.IdDocumento.ToString(), Regex.Replace(protoIn.Oggetto, @"\r\n?|\n", "-"), protoIn.Corpo, _vert.MittenteMailPec, Operatore, ruolo, _vert.CodiceAmministrazione);
                ...
                if (datiProto.Mittenti.Anagrafe.Count >= 1)
                {
                    foreach (var protoAnagrafe in datiProto.Mittenti.Anagrafe)
                    {
                        if (mittentiDestinatariList.Where(x => x.CodiceFiscale == protoAnagrafe.CODICEFISCALE || x.CodiceFiscale == protoAnagrafe.PARTITAIVA).Count() == 0)
                        {
                            ...
                            //Setto il campo DataRicevimento che, in seguito ai test fatti a Cesena, è necessario settare
                            //nel caso in cui il flusso è in A
                            if (DatiProtocollo.TipoAmbito == ProtocolloEnum.AmbitoProtocollazioneEnum.DA_ISTANZA)
                                mittentiDestinatari.DataRicevimento = DatiProtocollo.Istanza.DATA.Value.ToString("dd/MM/yyyy");

                            if (this.DatiProtocollo.TipoAmbito == ProtocolloEnum.AmbitoProtocollazioneEnum.DA_MOVIMENTO)
                                mittentiDestinatari.DataRicevimento = DatiProtocollo.Movimento.DATA.Value.ToString("dd/MM/yyyy");
        ...
        {
            _protocolloLogs.DebugFormat("Inizio metodo LeggiProtocolloDocumento, idprotocollo {0}, annoprotocollo {1}, numeroprotocollo {2}", idProtocollo,annoProtocollo, numeroProtocollo);
            if (!string.IsNullOrEmpty(idProtocollo) || (!string.IsNullOrEmpty(numeroProtocollo) && !string.IsNullOrEmpty(annoProtocollo)))
            {
                ...
                GC.Collect();
                if (String.IsNullOrEmpty(idProtocollo) || _vert.UsaNumAnnoLeggi)
                {
                    ...
                    _protocolloLogs.Debug("Fine Chiamata a LeggiProtocollo");
                }
                else
                {

...
namespace Init.SIGePro.Sit
{
	...
	public class SIT_SILVERBROWSER: SitBaseV2
	{
		...
		public override RetSit ElencoCivici()
		{
			...
			try
			{
				var civici = this._client.ListaCivici(new CodiceViario(codiceVia)).Select(x => x.numero).SeContiene(this.DataSit.Civico);
		...
		public override RetSit ElencoEsponenti()
		{
			...
			try
			{
				var esponenti = this._client
                                    ...
                                    .SeContiene(this.DataSit.Esponente);
		...
		public override RetSit ElencoSub()
		{
			...
			try
			{
				var sub = this._client.ListaSub(sezione, foglio, particella).Select(x => x.sub).SeContiene(this.DataSit.Sub); ;

...
{
	/// <summary>
	/// Descrizione di riepilogo per MySqlProvider.
	/// </summary>
	internal class MySqlProvider : IProvider
	{
		...
		public string NvlFunction(string columnName, object value)
		{
			...
			{
				return "IFNULL(" + columnName + ",'" + value + "')";
			}
			else
			{
				return "IFNULL(" + columnName + "," + value + ")";
		...
		{
			return String.Format("CONVERT( {0} , UNSIGNED INTEGER )", columnName);
		}
		...
		public string ClobLike(string columnName, string nomeParametro, bool confrontoUCase)
		{
			...
			return string.Format(" ( {0} like CONCAT('%' ,{1}, '%') ) ", columnName, QueryParameterName(nomeParametro));
		}

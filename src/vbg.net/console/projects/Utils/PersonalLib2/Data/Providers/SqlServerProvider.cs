...
{
	/// <summary>
	/// Descrizione di riepilogo per SqlServerProvider.
	/// </summary>
	internal class SqlServerProvider : IProvider
	{
		...
		public string NvlFunction(string columnName, object value)
		{
			...
			{
				return "ISNULL(" + columnName + ",'" + value + "')";
			}
			else
			{
				return "ISNULL(" + columnName + "," + value + ")";
		...
		{
			return string.Format("CONVERT(INT, {0})",columnName);
		}
		...
		{
			return string.Format(" LEN({0}) ", columnName);
		}

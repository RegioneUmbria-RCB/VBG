			...
			try
			{
                var sql = $@"select 
                            ...
                            where 
                                ...
                                vw_scadenzario.idcomune={m_database.Specifics.QueryParameterName("idComune")} and 
                                vw_scadenzario.CODICESCADENZA = {m_database.Specifics.QueryParameterName("codiceScadenza")}";
...
where 
    istanze.idcomune = vw_scadenzario.idcomune AND
    istanze.codiceistanza = vw_scadenzario.codiceistanza and 
    statiistanza.idcomune = vw_scadenzario.idcomune AND 
    statiistanza.software = vw_scadenzario.software AND
    statiistanza.codicestato = vw_scadenzario.codstatoistanza
    and (flag_blocca_integr_online is null or flag_blocca_integr_online='' or flag_blocca_integr_online=0)
    and vw_scadenzario.idcomune='{SafeString(m_idComune)}'
    and vw_scadenzario.SOFTWARE ='{SafeString(filtri.CodSportello)}' ");
			...
			{
                sql.AppendFormat($" and vw_scadenzario.AMM_PARTITAIVA ='{0}' ");

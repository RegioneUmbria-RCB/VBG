		...
		*/
        ...
        public void UpdateImporto(string idComune, int idOnere, double prezzo)
        {
            ...
            try
            {
                ...
                var sql = PreparaQueryParametrica("update ISTANZEONERI set prezzo = {0} where idcomune = {1} and id = {2}", "prezzo", "idComune", "id");

                using (IDbCommand cmd = db.CreateCommand(sql))
                {
                    cmd.Parameters.Add(db.CreateParameter("prezzo", prezzo));
                    cmd.Parameters.Add(db.CreateParameter("idComune", idComune));
                    cmd.Parameters.Add(db.CreateParameter("id", idOnere));

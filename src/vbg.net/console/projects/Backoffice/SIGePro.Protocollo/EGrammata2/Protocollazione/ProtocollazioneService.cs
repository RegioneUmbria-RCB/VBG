        ...
        public Risposta Protocollazione(ProtocollaAllegatiRequestType request)
        {
            try
            {
                using (var ws = CreaWebService())
                {
                    ...
                    Logs.InfoFormat("PROTOCOLLAZIONE AVVENUTA CON SUCCESSO, numero protocollo: {0}, anno protocollo: {1}, messaggio: {2}", response.NumeroProtocollo.numero, response.NumeroProtocollo.anno, response.Stato.Messaggio);

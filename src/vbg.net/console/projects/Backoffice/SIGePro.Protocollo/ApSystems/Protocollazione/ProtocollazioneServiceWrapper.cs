...
namespace Init.SIGePro.Protocollo.ApSystems.Protocollazione
{
    ...
    {
        public ProtocollazioneServiceWrapper(ProtocolloLogs logs, ProtocolloSerializer serializer, string username, string password, string url, string operatore)
        ...
        public DatiProtocolloRes ProtocollaPartenzaBozza(DataSet request)
        {
            try
            {
                using (var ws = CreaWebService())
                {
                    ...
                    {
                        IdProtocollo = ds.protocollo[0].codice,
                        AnnoProtocollo = DateTime.Parse(ds.protocollo[0].data_protocollo_generale).ToString("yyyy"),
                        DataProtocollo = ds.protocollo[0].data_protocollo_generale,

...
namespace Init.SIGePro.Protocollo.SiprWebTest.Verticalizzazioni
{
    public class VerticalizzazioniConfiguration
    {
        ...
        public VerticalizzazioniConfiguration(ProtocolloLogs logs, VerticalizzazioneProtocolloSiprwebtest vert)
        {
            if (!vert.Attiva)
                throw new Exception("La verticalizzazione PROTOCOLLO_SIPRWEBTEST non è attiva");

            _logs = logs;
            EstraiParametri(vert);
        }

        private void VerificaIntegritaParametri(VerticalizzazioneProtocolloSiprwebtest paramVert)
        {
            try
            {
        ...
        private void EstraiParametri(VerticalizzazioneProtocolloSiprwebtest vert)
        {
            try
            {
            ...
            {
                throw new Exception("ERRORE GENERATO DURANTE IL RECUPERO DEI VALORI DALLA VERTICALIZZAZIONE PROTOCOLLO_SIPREWBTEST", ex);
            }
        }
    }
}

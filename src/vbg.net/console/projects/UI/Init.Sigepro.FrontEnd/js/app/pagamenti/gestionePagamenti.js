/*global define*/
/*jslint unparam: true,*/
define(['app/pagamenti/TabellaRiepilogoOneri', 'jquery', 'jquery.ui'], function (TabellaRiepilogoOneri, $) {
    'use strict';
    ...
    function ricalcolaTotali() {
        ...
        $('.ddlPagamento').each(function (idx, element) {
            ...
            if (valore === Constants.IdValoreGiaPagato || valore === Constants.IdValorePagaOnline) {
                daPagare = parseFloat(txtImportoPagato.val().replace(',', '.'));
    ...
    function init() {
        ...
        $('.helpNoteOnere').click(function (e) {
            $('.note-onere .modal-body').html($(this).parent().find('.noteOnere').html());
            $('.note-onere').modal('show');
            e.preventDefault();
        });
    }

...
namespace SIGePro.Net.WebServices.WsSIGePro
{
    ...
    public class IstanzeWs : SigeproWebService
    {
        ...
        [WebMethod]
        [OperationContract]
        public Init.SIGePro.Data.Istanze GetDettaglioPraticaByUuid(string token, string uuid)
        {
            AuthenticationInfo ai = AuthenticationManager.CheckToken(token);
            ...
            try
            {
                using (var db = ai.CreateDatabase())
                {
                    ...
                    var codiceIstanza = mgr.GetCodiceIstanzaDaUuid(ai.IdComune, uuid);

                    var istanza = mgr.GetById(ai.IdComune, codiceIstanza, useForeignEnum.Recoursive);

                    // Leggo eventuali poratiche collegate
                    var verticalizzazioneStc = new VerticalizzazioneStc(ai.Alias, istanza.SOFTWARE);
                    var subVisuraService = new SubvisuraPraticheService(db, verticalizzazioneStc);
                    ...
                    {
                        var uidPraticaCollegata = subVisuraService.GetUdiPraticaCollegataDaIdMovimento(movimento);

...
namespace Init.Sigepro.FrontEnd.AppLogic.GenerazioneDocumentiDomanda.GenerazioneCertificatoDiInvio
{
	...
	public class CertificatoDiInvioService
	{
		...
		public BinaryFile GeneraCertificatoDiInvio(string idDomandaBackoffice)
		{
			try
			{
				var fileCertificato = this._generatoreCertificatoDiInvio.GeneraCertificatoDiInvio(idDomandaBackoffice, this._strategiaIndividuazioneRiepilogo);
			...
			{
				_log.ErrorFormat("Generazione del certificato di invio fallita per la domanda (id domanda backoffice {0}): {1}", idDomandaBackoffice, ex.ToString());
		...
		public int? GetCodiceOggettoCertificatoDiInvioDaIdDomandaBackoffice(string idDomandaBackoffice)
		{
			try
			{
				...
				return this._certificatoDiInvioAllegato.GetIdCertificatoDiInvio(idDomandaBackoffice);
			}
			catch (Exception ex)
			{
				_log.ErrorFormat("Errore durante la lettura del certificato di invio allegato alla domanda backoffice con id {0}: {1}", idDomandaBackoffice, ex.ToString());

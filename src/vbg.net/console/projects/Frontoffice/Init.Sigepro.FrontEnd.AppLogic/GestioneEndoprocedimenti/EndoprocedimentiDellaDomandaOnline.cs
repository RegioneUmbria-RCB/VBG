...
namespace Init.Sigepro.FrontEnd.AppLogic.GestioneEndoprocedimenti
{
	...
	public class EndoprocedimentiDellaDomandaOnline
	{
        ...
        internal EndoprocedimentiDellaDomandaOnline(EndoprocedimentiConsole endoConsole)
        {
            this.FamiglieEndoprocedimentiPrincipali = endoConsole.Principali?.ToList() ?? new List<FamigliaEndoprocedimentoDto>();
            this.FamiglieEndoprocedimentiAttivati = endoConsole.Richiesti?.ToList() ?? new List<FamigliaEndoprocedimentoDto>();
            this.FamiglieEndoprocedimentiAttivabili = endoConsole.Ricorrenti?.ToList() ?? new List<FamigliaEndoprocedimentoDto>();
            this.FamiglieEndoprocedimentiFacoltativi = endoConsole.Altri?.ToList() ?? new List<FamigliaEndoprocedimentoDto>();

            this.EndoPrincipali = this.FamiglieEndoprocedimentiPrincipali.SelectMany(x => x.TipiEndoprocedimenti.SelectMany(y => y.Endoprocedimenti)).ToList();
            this.EndoAttivati = this.FamiglieEndoprocedimentiAttivati.SelectMany(x => x.TipiEndoprocedimenti.SelectMany(y => y.Endoprocedimenti)).ToList();
            this.EndoAttivabili = this.FamiglieEndoprocedimentiAttivabili.SelectMany(x => x.TipiEndoprocedimenti.SelectMany(y => y.Endoprocedimenti)).ToList();

            this.IdSelezionatiDefault = PopolaListaIdSelezionatiPerDefault();

            this._logicaEstrazioneEndoFacoltativi = new LogicaEstrazioneEndoFacoltativi(this.FamiglieEndoprocedimentiFacoltativi.ToArray());
        }

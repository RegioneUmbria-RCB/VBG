...
namespace Init.Sigepro.FrontEnd.AppLogic.Repositories.WebServices
{
	internal class WsInterventiRepository : WsAreaRiservataRepositoryBase, IInterventiRepository
	{
		...
		public InterventoDto[] GetSottonodi(string aliasComune, string software, int idnodo, IAmbitoRicercaIntervento ambitoRicerca, LivelloAutenticazioneEnum livelloAutenticazione)
		{
			using (var ws = _serviceCreator.CreateClient(aliasComune))
			{
                var livello = (LivelloAutenticazioneBOEnum)((int)livelloAutenticazione);

				return ws.Service.GetSottonodiIntervento(ws.Token, software, idnodo, ambitoRicerca.GetAmbito(), livello);
			}
		}

		public InterventoDto[] GetSottonodiDaIdAteco(string aliasComune, string software, int idNodoPadre, int idAteco, IAmbitoRicercaIntervento ambitoRicerca, LivelloAutenticazioneEnum livelloAutenticazione)
		{
			using (var ws = _serviceCreator.CreateClient(aliasComune))
			{
                var livello = (LivelloAutenticazioneBOEnum)((int)livelloAutenticazione);

				return ws.Service.GetSottonodiInterventoDaIdAteco(ws.Token, software, idNodoPadre, idAteco, ambitoRicerca.GetAmbito(), livello);
			}
		}
		...
		#region IInterventiRepository Members
		...
		{
			var nodi = this.GetSottonodi(idComune, software, -1, new AmbitoRicercaAreaRiservata(false), LivelloAutenticazioneEnum.Identificato);
        ...
        public bool InterventoAttivo(int idIntervento, LivelloAutenticazioneEnum livelloAutenticazione, bool utenteTester)
		{
			if (utenteTester)
			{
				return true;
			...
			using (var ws = _serviceCreator.CreateClient(_aliasSoftwareResolver.AliasComune))
			{
				...
				return ws.Service.InterventoAttivoNellAreaRiservata(ws.Token, livello, idIntervento);
			}
		}

...
namespace Init.Sigepro.FrontEnd.AppLogic.Adapters
{
	public partial class IstanzaSigeproAdapter
	{
		...
		public Istanze Adatta()
		{
			...
			istanzaSigepro.ConfigurazioneComune = _configurazioneVbgRepository.LeggiConfigurazioneComune(aliasComune, software);
			/*
		...
		private void FillAnagrafiche(Istanze ist)
		{
			...
			foreach (var row in _readInterface.Anagrafiche.Anagrafiche)
			{
				...
				string codAventeProcura = _readInterface.Procure.GetCodiceFiscaleDelProcuratoreDi(row.Codicefiscale);
		...
		private void FillDocumenti(Istanze istanzaOut)
		{
			...
			if (_readInterface.DelegaATrasmettere.Allegato != null)
			{
				...
				{
					DOCUMENTO = String.IsNullOrEmpty(descrizioneDelega) ? "Delega a trasmettere" : descrizioneDelega,
					DATA = DateTime.Now,
					CODICEOGGETTO = _readInterface.DelegaATrasmettere.Allegato.CodiceOggetto.ToString(),
					Oggetto = new DocumentiIstanzaOggetti
					{
						NOMEFILE = _readInterface.DelegaATrasmettere.Allegato.NomeFile
					}
				};
				documenti.Add(doc);
			}
		...
		private void FillOneri(Istanze istanzaOut)
		{
			...
			istanzaOut.Oneri = _readInterface.Oneri.Oneri
													...
													{
														CODICEINVENTARIO	= x.Provenienza == OnereDaPagare.ProvenienzaOnereEnum.Endoprocedimento ? x.EndoOInterventoOrigine.ToString() : String.Empty,
														PREZZO				= x.Importo,
														FLENTRATAUSCITA		= "1",
														DATA				= DateTime.Now,

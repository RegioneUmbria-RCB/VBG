...
[assembly: WebResource("Init.Sigepro.FrontEnd.WebControls.Interventi.help_interventi.gif", "image/gif")]
[assembly: WebResource("Init.Sigepro.FrontEnd.WebControls.Interventi.InterventiTreeRenderer.js", "text/javascript")]
namespace Init.Sigepro.FrontEnd.WebControls.Interventi
{
	public class InterventiTreeRenderer: WebControl, INamingContainer
	{
		...
		{
			if (!this.Page.ClientScript.IsClientScriptIncludeRegistered(this.GetType(), "InterventiTreeRenderer"))
			{
				this.Page.ClientScript.RegisterClientScriptInclude(
				   this.GetType(), "InterventiTreeRenderer",
				   Page.ClientScript.GetWebResourceUrl(this.GetType(),
				   "Init.Sigepro.FrontEnd.WebControls.Interventi.InterventiTreeRenderer.js"));
			}
		...
		private void RenderSubNodes(List<NodoAlberoInterventi> nodiFiglio, HtmlTextWriter writer)
		{
			for (int i = 0; i < nodiFiglio.Count; i++)
			{
				...
				if (el.NodiFiglio.Count > 0)
				{
					...
					if (!HideFolders)
					{
						writer.AddAttribute("src", GetBaseUrl() + "Images/folder-closed.gif");
				...
				else
				{
					...
					if (!HideFolders)
					{
						writer.AddAttribute("src", GetBaseUrl() + "Images/file.gif");
						writer.RenderBeginTag(HtmlTextWriterTag.Img);
						writer.RenderEndTag();
					}
		...
		private void RenderNoteNodo(HtmlTextWriter writer, NodoAlberoInterventi el)
		{
			...
			var url = UrlIconaHelp;

			if(url == "auto")
				url =Page.ClientScript.GetWebResourceUrl(this.GetType(), @"Init.Sigepro.FrontEnd.WebControls.Interventi.help_interventi.gif");

...
namespace Init.Sigepro.FrontEnd.GestioneMovimenti.DatiDinamici
{
	...
	public class Dyn2DataAccessProvider : IDyn2DataAccessProvider
	{
		...
		public Dyn2DataAccessProvider(IDatiDinamiciRepository datiDinamiciRepository, string aliasComune, int idModello, ITokenApplicazioneService tokenService, DatiMovimentoDaEffettuare movimentoDaEffettuare, ICommandSender bus, IIstanzeManager istanzeManager, int codiceIstanza)
		{
			this._aliasComune = aliasComune;
			
			this._tokenService = tokenService;

			var cacheModelloDinamico = datiDinamiciRepository.GetCacheModelloDinamico(idModello);
			...
			this._istanzeDyn2DatiManager = new IstanzeDyn2DatiManager(movimentoDaEffettuare, bus);
		...
		{
			var istanza = (Istanze)this._istanzeManager.LeggiIstanza(this._aliasComune, this._codiceIstanza);

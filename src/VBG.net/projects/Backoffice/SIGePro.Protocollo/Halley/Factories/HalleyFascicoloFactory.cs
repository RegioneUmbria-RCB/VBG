...
namespace Init.SIGePro.Protocollo.Halley.Factories
{
    ...
    {
        public static IFascicoloHalleyBuilder Create(ResolveDatiProtocollazioneService datiProtocollazioneService, HalleyVerticalizzazioneParametriAdapter vert, string tokenDizionario, ProtocolloLogs log)
        {
            ...
            else if (datiProtocollazioneService.TipoAmbito == ProtocolloEnumerators.ProtocolloEnum.AmbitoProtocollazioneEnum.DA_MOVIMENTO)
            {
                ...
                return new HalleyFascicoloMovimentoBuilder(datiProtocollazioneService.Istanza.NUMEROPROTOCOLLO, datiProtocollazioneService.Istanza.DATAPROTOCOLLO.Value.ToString("yyyy"), log, vert, tokenDizionario);
            }
            else if (datiProtocollazioneService.TipoAmbito == ProtocolloEnumerators.ProtocolloEnum.AmbitoProtocollazioneEnum.NESSUNO)
                return null;
            else

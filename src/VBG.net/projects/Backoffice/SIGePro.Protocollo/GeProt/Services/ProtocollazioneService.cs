...
namespace Init.SIGePro.Protocollo.GeProt.Services
{
    public class ProtocollazioneService
    {
        ...
        private string VerificaEsistenzaFile(string path, int indice = 0)
        {
            ...
            {
                indice += 1;
                retVal = VerificaEsistenzaFile(Path.Combine(directory, String.Format("Copia ({0}) - {1}", indice, fileName)), indice);
            }
        ...
        public void InserisciAllegato(ProtocolloAllegati all, ProtocolloGeProtService ws)
        {
            try
            {
                ...
                path = VerificaEsistenzaFile(path);

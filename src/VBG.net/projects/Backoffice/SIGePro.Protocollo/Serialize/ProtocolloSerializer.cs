...
namespace Init.SIGePro.Protocollo.Serialize
{
    public class ProtocolloSerializer
    {
        ...
        public object Deserialize(string xmlString, Type type)
        {
            ...
            MemoryStream memStreamOutput = null;

            memStreamOutput = new MemoryStream();
            memStream = StreamUtils.StringToStream(xmlString);
            StreamUtils.BulkTransfer(memStream, memStreamOutput);
            memStream.Seek(0, SeekOrigin.Begin);

            var xmlSerializer = new XmlSerializer(type);
            response = xmlSerializer.Deserialize(memStreamOutput);

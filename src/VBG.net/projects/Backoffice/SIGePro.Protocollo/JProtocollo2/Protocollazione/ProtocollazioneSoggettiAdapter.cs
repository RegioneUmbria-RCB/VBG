...
namespace Init.SIGePro.Protocollo.JProtocollo2.Protocollazione
{
    public class ProtocollazioneSoggettiAdapter
    {
        ...
        {
            var amministrazioni = _datiProto.AmministrazioniEsterne.Select(x => new soggetto { denominazione = x.AMMINISTRAZIONE, indirizzo = x.INDIRIZZO });
            var anagrafica = _datiProto.AnagraficheProtocollo.Select(x => new soggetto { denominazione = x.GetNomeCompleto() });

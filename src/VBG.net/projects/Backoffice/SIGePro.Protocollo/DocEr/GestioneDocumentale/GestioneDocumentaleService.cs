    ...
    public class GestioneDocumentaleService
    {
        ...
        private bool SetAclDocument(DocerServicesPortTypeClient ws, IAuthenticationService auth, string docId)
        {
            try
            {
                ...
                var response = ws.setACLDocument(_token, docId, ruoli);
        ...
        public long InserisciDocumentoPrimario(IAuthenticationService auth, ProtocolloAllegati allegatoPrimario, string typeId, string codiceEnte, string codiceAoo, string tipoDocPrincipale, ResolveDatiProtocollazioneService datiProtoService)
        {
            try
            {
                using (var ws = CreaWebService())
                {
                    var metadati = new GestioneDocumentaleMetadataAdapter(allegatoPrimario, datiProtoService, typeId, codiceEnte, codiceAoo, tipoDocPrincipale);
                    var keys = metadati.Adatta();
        ...
        public void InserisciDocumentiAllegati(IAuthenticationService auth, string docPrimario, IEnumerable<ProtocolloAllegati> docsAllegati, string typeId, string codiceEnte, string codiceAoo, string tipoDocAllegati, ResolveDatiProtocollazioneService datiProtoService)
        {
            try
            {
                ...
                using (var ws = CreaWebService())
                {
                    ...
                    foreach (var doc in docsAllegati)
                    {
                        ...
                        var metadati = new GestioneDocumentaleMetadataAdapter(doc, datiProtoService, typeId, codiceEnte, codiceAoo, tipoDocAllegati);
                        var keys = metadati.Adatta();
        ...
        public void SetAclFascicolo(Fascicolo datiFasc, IAuthenticationService auth, string codiceEnte, string codiceAoo)
        {
            try
            {
                using (var ws = CreaWebService())
                {
                    ...
                    if (ruoli == null)
                    {
                        _logs.WarnFormat("ATTENZIONE, L'UTENTE {0} NON HA SETTATO ALCUN RUOLO IN QUANTO IL DATO PROVIENE DALLA VERTICALIZZAZIONE", auth.Username);
                        return;
                    }
                    ...
                    var response = ws.setACLFascicolo(_token, metadati, ruoli);

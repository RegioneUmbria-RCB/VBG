...
using Init.SIGePro.Protocollo.Iride.Builder;
...
namespace Init.SIGePro.Protocollo
{
    ...
    public class PROTOCOLLO_IRIDE : ProtocolloBase
    {
        ...
        private string _mittenteMailPec = ""; //compesarotest@emarche.it
        ...
        private DatiFascicolo FascicolaProtocollo(Fascicolo fascicolo)
        {
            ...
            if (this.DatiProtocollo.TipoAmbito == ProtocolloEnum.AmbitoProtocollazioneEnum.NESSUNO)
            {
                ...
                EsitoOperazione esito = _proxyFascIride.FascicolaDocumento(iFascicoloId, iDocumentoId, _aggiornaClassifica, Operatore.ToUpper(), Ruolo, _codiceAmministrazione, string.Empty);
            ...
            if (this.DatiProtocollo.TipoAmbito == ProtocolloEnum.AmbitoProtocollazioneEnum.DA_ISTANZA)
            {
                ...
                foreach (Movimenti elem in list)
                {
                    if (!string.IsNullOrEmpty(elem.FKIDPROTOCOLLO) || (!string.IsNullOrEmpty(elem.NUMEROPROTOCOLLO) && elem.DATAPROTOCOLLO.HasValue))
                    {
                        ...
                        {
                            _protocolloLogs.DebugFormat("Chiamata a FascicolaDocumento, IdFascicolo: {0}, IdDocumento: {1}, AggiornaClassifica: {2}, Operatore: {3}, Ruolo: {4}, CodiceAmministrazione: {5}, CodiceAoo: {6}", iFascicoloId, iDocumentoId, _aggiornaClassifica, Operatore.ToUpper(), Ruolo, _codiceAmministrazione, String.Empty);
                            esito = _proxyFascIride.FascicolaDocumento(iFascicoloId, iDocumentoId, _aggiornaClassifica, Operatore.ToUpper(), Ruolo, _codiceAmministrazione, string.Empty);
                            if (!esito.Esito)
                                continue;
                        }
                    }
        ...
        */
        ...
        private void InviaPec(DatiProtocolloIn protoIn, ProtocolloOut protoOut, string ruolo)
        {
            ...
            else
            {
                ...
                string response = pecSrv.InviaPECInterop(postePecBuilder.SegnaturaXml, _codiceAmministrazione, String.Empty);
            }
        }
        ...
        private void SetAllegati(ProtocolloIn protoIn, DatiProtocolloIn datiProtoIn)
        {
            try
            {
                ...
                foreach (ProtocolloAllegati protoAllegati in datiProtoIn.Allegati)
                {
                    ...
                    /*
                    int iPos = pProtAllegati.Descrizione.LastIndexOf("." + pProtAllegati.Extension);
                    if (iPos != -1)
                        pProtocolloIn.Allegati[iIndex].Commento = pProtAllegati.Descrizione;
                    else
                        pProtocolloIn.Allegati[iIndex].Commento = pProtAllegati.NOMEFILE;
                    */
                    ...
                    if (response.Allegati != null && response.Allegati.Length > 0)
                    {
                        ...
                        foreach (var a in response.Allegati)
                        {
                            ...
                            allegatiList.Add(new AllOut
                            {
                                ...
                                Image = a.Image,
                                TipoFile = String.IsNullOrEmpty(a.TipoFile) ? String.Empty : a.TipoFile,
                                ContentType = String.IsNullOrEmpty(a.TipoFile) ? String.Empty : new OggettiMgr(this.DatiProtocollo.Db).GetContentType(a.Commento)
                            });
                        }
            ...
            try
            {
                ...
                if (protocolloIride.Attiva)
                {
                    _protocolloLogs.DebugFormat(@"Valori parametri verticalizzazioni: url: {0}, 
                                                                                     ...
                                                                                     Mittente Pec: {11},
                                                                                     Url Pec: {12}, 
                                                                                     Usa numero e anno su leggi proto: {13}",
                    protocolloIride.Url,

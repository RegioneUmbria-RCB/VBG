...
namespace Init.SIGePro.DatiDinamici.WebControls
{
	...
	public partial class ModelloDinamicoRenderer : WebControl, INamingContainer
	{
		...
		*/
		...
		// CONTROLLI FIGLIO
		HtmlTable m_table = new HtmlTable();	// tabella utilizzata per contenere la vista del modello
...
//			m_table.ID = "_table";
		...
		public override void DataBind()
		{
			...
			var renderer = new ModelloDinamicoTableRenderer(this.DataSource, this.MascheraSolaLettura , this.CompatibilitaOpenOffice, callbackAggiuntaRighe, callbackEliminazioneRighe);
		...
		protected override void OnPreRender(EventArgs e)
		{
			...
			if (this.DesignMode)
			{
				...
				for (int i = 0; i < 5; i++)
				{
					m_table.Rows.Add(new HtmlTableRow());

					for (int j = 0; j < 3; j++)
					{
						Label lbl = new Label();
						lbl.Text = "Etichetta " + i;
						m_table.Rows[i].Cells.Add(new HtmlTableCell());
						m_table.Rows[i].Cells[(j * 2)].Controls.Add(lbl);

						TextBox txt = new TextBox();
						txt.Text = "Controllo " + i;
						m_table.Rows[i].Cells.Add(new HtmlTableCell());
						m_table.Rows[i].Cells[(j * 2) + 1].Controls.Add(txt);
		...
		{
			writer.AddAttribute("class", "DatiDinamici");

		...
		public int InsertOggetto(string fileName, string mimeType, byte[] fileData)
		{
			...
			try
			{
				using (var ws = CreateClient())
				{
					...
					_log.DebugFormat("Parametri di chiamata: {0}", StreamUtils.SerializeClass(req));

					var res = ws.OggettiInsert(req);

					_log.DebugFormat("Risultato della chiamata: {0}", res.id);
		...
		public void UpdateOggetto(int id, string fileName,byte[] fileData)
		{
			try
			{
				using (var ws = CreateClient())
				{
					...
					_log.DebugFormat("Risultato della chiamata: {0}", res.result);
				}
			}
			catch (Exception ex)
			{
		...
		public string GetFileName(int id)
		{
			try
			{
				using (var ws = CreateClient())
				{
					...
					_log.DebugFormat("Risultato della chiamata: {0}", res.fileName);
		...
		public OggettiFindResponse GetByIdNativo(int id)
		{
			try
			{
				using (var ws = CreateClient())
				{
					...
					_log.DebugFormat("Risultato della chiamata: {0}", StreamUtils.SerializeClass(res));

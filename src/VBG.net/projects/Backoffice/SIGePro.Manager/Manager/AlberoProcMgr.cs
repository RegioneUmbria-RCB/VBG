namespace Init.SIGePro.Manager
{
	...
	public partial class AlberoProcMgr
	{
		...
		public ClassTree<AlberoProc> RisaliStrutturaAlbero(string idComune, int idIntervento)
		{
			var ramoAlbero = this.GetById(idIntervento, idComune);
				...
				using (IDbCommand cmd = db.CreateCommand(sql))
				{
					...
					using (var dr = cmd.ExecuteReader())
					{
						while (dr.Read())
						{
							...
							var schedaPresente = schedeIntervento.FirstOrDefault(x => x.Codice == idModello);
		...
		public List<FamigliaEndoprocedimentoDto> GetEndoDaIdIntervento(string idComune, int codIntervento, AmbitoRicerca ambitoRicerca)
		{
			...
			try
			{
				...
				sql += " order by alberoproc_endo.flag_principale desc,alberoproc_endo.flag_richiesto desc,tipiendo.ordine,inventarioprocedimenti.codicetipo,inventarioprocedimenti.ordine";
		...
		/// </summary>
		public List<InterventoDto> GetNodiFiglio(string idComune, string software, int idNodo, AmbitoRicerca ambito, LivelloAutenticazioneBOEnum livelloAutenticazione)
		{
			...
			var sql = @"SELECT 
                        ...
                        AND (LIVELLO_AUTENTICAZIONE is null or LIVELLO_AUTENTICAZIONE <= {3})
						and (FINE_VALIDITA is null or {4} between INIZIO_VALIDITA and FINE_VALIDITA)
						and " + db.Specifics.LengthFunction("sc_codice") + "=" + (scCodice.Length + 2) + @"
						order by SC_ORDINE ASC,SC_DESCRIZIONE asc";

			sql = PreparaQueryParametrica(sql, "idComune", "software", "scCodiceLike", "livelloAutenticazione", "data");
			...
			try
			{
				...
				using (IDbCommand cmd = db.CreateCommand(sql))
				{
                    ...
                    cmd.Parameters.Add(db.CreateParameter("livelloAutenticazione", livelloAutenticazione));

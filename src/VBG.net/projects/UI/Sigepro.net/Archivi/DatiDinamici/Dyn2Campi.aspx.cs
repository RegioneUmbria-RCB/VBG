...
public partial class Archivi_DatiDinamici_Dyn2Campi : BasePage
{
	...
	private void BindGrigliaProprieta()
	{
		...
		var proprieta = ControlliDatiDinamiciDictionary.Items[tipoDato].GetProprietaEditabili();
		var proprietaValorizzate = new Dyn2CampiMgr(Database).GetProprietaControllo(IdComune, id );

		proprietaValorizzate.ForEach(delegate(Dyn2CampiProprieta prop)
		{
			if (proprieta.ContainsKey(prop.Proprieta))
				proprieta[prop.Proprieta].Value = prop.Valore;
		});

		gvProprietaControllo.DataSource = proprieta.Values;
		gvProprietaControllo.DataBind();

		var helpControllo = ControlliDatiDinamiciDictionary.Items[tipoDato].GetDescrizioneControllo();

...
namespace Init.Sigepro.FrontEnd.Reserved.InserimentoIstanza
{
	public partial class GestioneOneri : IstanzeStepPage
	{
		...
		public bool ContieneOneriIntervento
		{
			get 
			{
				return ReadFacade.Domanda.Oneri.Oneri.Where(x => x.Provenienza == OnereDaPagare.ProvenienzaOnereEnum.Intervento).Count() > 0;
			}
		}
		...
		public bool ContieneOneriEndo
		{
			get
			{
				return ReadFacade.Domanda.Oneri.Oneri.Where( x => x.Provenienza == OnereDaPagare.ProvenienzaOnereEnum.Endoprocedimento).Count() > 0;
			}
		}
		#endregion
		...
		public override void DataBind()
		{
			...
			if (!new DomandaContieneAttestazioneDiPagamento().IsSatisfiedBy( ReadFacade.Domanda ))
		...
		private void MostraVistaDettaglioFile()
		{
			...
			hlFileCaricato.NavigateUrl = ResolveClientUrl(String.Format(url, IdComune , Software , codiceOggetto , UserAuthenticationResult.Token , IdDomanda));
